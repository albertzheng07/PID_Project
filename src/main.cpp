      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          steer_value = clip(pid_steer.TotalError(),-1.0,1.0); // clip steering input to max value
          double speed_error = 15.0-speed;
          pid_throttle.UpdateError(speed_error);
          throttle_value = clip(pid_throttle.TotalError(),0.0,1.0); // clip steering input to max value
